...
async function fetchAdminData() {
    if (!token) return;
    try {
        // --- UPDATED: Fetch /me first to check role ---
        const meRes = await fetch(`${baseURL}/me`, { headers: getAuthHeaders() });
        ...
        // Find the User Management link by its ID
        const userManagementLink = document.getElementById("nav-user-management");
        
        if (userRole !== 'admin') {
            // If the user is NOT an admin (i.e., they are 'faculty'),
            // hide the User Management link.
            if (userManagementLink) {
                userManagementLink.style.display = 'none';
            }
        }
        // If they ARE an admin, the link will show by default.
        ...
        // --- UPDATED: Fetch analytics after role check ---
        const analyticsRes = await fetch(`${baseURL}/analytics/summary`, { headers: getAuthHeaders() });
        if (analyticsRes.status === 401) return logout(); // This will now correctly fail for 'student'
        ...
        // If a faculty logs in, analytics will load. 
        // If a student somehow gets here, this will fail, which is good.
    }
}
...
async function loadUsers() {
    try {
        ...
        // If a faculty member tries to load this, the server will send 403 Forbidden
        if (res.status === 403) {
...
document.addEventListener("DOMContentLoaded", () => {
    const path = window.location.pathname.split('/').pop(); // Get the file name
    
    // This logic runs the correct setup function for the page you are on.
    if (path === 'dashboard.html') {
        checkAuth();
        fetchStudentData();
    } else if (path === 'admin_dashboard.html') {
        checkAuth();
        fetchAdminData(); // This will fetch admin data and check roles
    } else if (path === 'user_management.html') {
        checkAuth();
        fetchAdminData(); // This will check roles and hide links
        loadUsers(); // This will try to load users (and fail if faculty)
    } else if (path === 'reports.html') {
        checkAuth();
        fetchAdminData(); // This will check roles and hide links
        loadAnalytics();
    } else if (path === 'index.html' || path === '') {
        // No auth check needed for login page
    }
});
...
# Base.metadata.create_all(bind=engine) # <-- This is moved to startup
...
def require_admin_auth(current_user: User = Depends(get_current_user)):
    if current_user.role not in ['admin', 'faculty']:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Admin/Faculty privileges required")
    return current_user
...
def get_analytics_summary(admin: User = Depends(require_admin_auth), db: Session = Depends(get_db)):
